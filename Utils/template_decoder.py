...
def is_valid_pred(p_site, p_class, len_a, len_a_temps):
    return False if p_site is None or p_class == 0 or p_site >= len_a and \
            p_class <= len_a_temps else True


def read_prediction(args, smiles, prediction, atom_templates, bond_templates, template_infos):
    pred_site = None
    template = '>>'
    temp_infos = {}
    predicion_score = 0
    ...
    if len(prediction) > 1:
        (p_site, pred_class), prediction_score = eval(prediction) 
        atoms, bonds = get_edit_site(mol)
        idx_map = get_idx_map(mol)
        if is_valid_pred(p_site, pred_class, len(atoms), \
                         len(atom_templates)):
            if pred_class <= len(atom_templates):
                pred_site, template = atoms[p_site], atom_templates[pred_class]
                if len(template.split('>>')[0].split('.')) > 1: 
                    pred_site = idx_map[pred_site] 
            else:
                pred_site, template = bonds[p_site], bond_templates[pred_class]
                if len(template.split('>>')[0].split('.')) > 1:
                    pred_site= (idx_map[pred_site[0]], idx_map[pred_site[1]])
            temp_infos = template_infos[template]
        [atom.SetAtomMapNum(atom.GetIdx()) for atom in mol.GetAtoms()]
    return mol, pred_site, template, temp_infos, prediction_score
